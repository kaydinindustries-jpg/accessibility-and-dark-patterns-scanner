...
const stripe = STRIPE_SECRET_KEY ? new StripeLib(STRIPE_SECRET_KEY) : null;
...
const PAGE_TIMEOUT_MS = process.env.PAGE_TIMEOUT_MS ? Number(process.env.PAGE_TIMEOUT_MS) : 30000;
...
const scansMem = [];
const findingsMem = new Map();
const alertsMem = [];
// In-memory auth stores for dev mode (avoid Postgres dependency during OIDC_DEV_MODE)
const accountsMem = new Map(); // key: `${provider}:${sub}` -> userId
const usersMem = new Map();
const usersByEmailMem = new Map();
...
async function issueSession(userId, email, ttlSec = 7 * 24 * 3600) {
    const jti = uuidv4();
    const now = Math.floor(Date.now() / 1000);
    const jwt = await new SignJWT({ uid: userId, email })
...
async function getSessionFromReq(req) {
    try {
        ...
        if (!IN_MEMORY_MODE && jti) {
            ...
            if (r.rows.length && (r.rows[0].revoked === true || new Date(r.rows[0].expires_at) < new Date()))
                return null;
        }
...
async function requireOrgMemberFromReq(req, res, orgId) {
    if (!AUTH_ENABLED)
        return true;
...
async function verifyProviderIdToken(provider, idToken, expectedNonce) {
    let jwksUrl;
    if (provider === 'google') {
    ...
    if (provider === 'google' && !(Array.isArray(aud) ? aud.includes(OIDC_GOOGLE_CLIENT_ID) : aud === OIDC_GOOGLE_CLIENT_ID))
        throw new Error('invalid_audience');
    if (provider === 'microsoft' && !(Array.isArray(aud) ? aud.includes(OIDC_MS_CLIENT_ID) : aud === OIDC_MS_CLIENT_ID))
        throw new Error('invalid_audience');
    if (!nonce || nonce !== expectedNonce)
        throw new Error('invalid_nonce');
    return { sub: payload.sub, email, name, iss: payload.iss };
}
async function upsertUserAndAccount(provider, sub, email, name) {
    if (IN_MEMORY_MODE) {
        ...
        if (!userId) {
            ...
            if (email)
                usersByEmailMem.set(email, userId);
        }
        else {
    ...
    if (acc.rows.length) {
        const userId = acc.rows[0].user_id;
        if (email || name)
            await pool.query(`UPDATE users SET email = COALESCE($2,email), name = COALESCE($3,name) WHERE id = $1`, [userId, email || null, name || null]);
        return userId;
    }
    let userId = null;
    if (email) {
...
let connection = null;
let scansQueue = null;
let scansEvents = null;
let exportsQueue = null;
let exportsEvents = null;
if (!IN_MEMORY_MODE) {
    const IORedisCjs = require('ioredis');
    connection = new IORedisCjs(REDIS_URL, { maxRetriesPerRequest: null });
    scansQueue = new Queue('scans', { connection });
    scansEvents = new QueueEvents('scans', { connection });
    exportsQueue = new Queue('exports', { connection });
    exportsEvents = new QueueEvents('exports', { connection });
}
// Scheduling helpers
async function getLatestScanTemplateForSite(siteId) {
    if (IN_MEMORY_MODE)
        return null;
    const r = await pool.query(`SELECT meta FROM scans WHERE site_id = $1 AND status = 'completed' ORDER BY created_at DESC LIMIT 1`, [siteId]);
    if (r.rows.length === 0)
        return null;
    const meta = r.rows[0].meta || {};
    const urls = Array.isArray(meta.urls) ? meta.urls : [];
    const viewport = meta.viewport;
    if (!urls.length || !viewport)
        return null;
    return {
        urls,
        viewport,
...
function buildRepeatOpts(s) {
    if (s.type === 'every') {
        if (!s.every_ms || s.every_ms <= 0)
            throw new Error('invalid_every_ms');
        const repeat = { every: Number(s.every_ms) };
        if (s.limit && s.limit > 0)
            repeat.limit = s.limit;
        return repeat;
    }
    else {
        if (!s.cron)
            throw new Error('invalid_cron');
        const repeat = { pattern: s.cron };
        if (s.timezone) {
            // BullMQ v5 uses tz for cron-parser options
            repeat.tz = s.timezone;
        }
        if (s.limit && s.limit > 0)
            repeat.limit = s.limit;
        return repeat;
...
async function upsertSchedulerForRow(row) {
    if (IN_MEMORY_MODE || !scansQueue)
        return;
    if (!row.active)
        return;
    ...
    await scansQueue.upsertJobScheduler(row.id, repeatOpts, {
        name: 'scheduled-scan',
        data: {
            orgId: row.org_id,
            siteId: row.site_id,
...
async function removeSchedulerById(id) {
    if (IN_MEMORY_MODE || !scansQueue)
        return false;
    try {
...
function percentile50(sorted) { const m = Math.floor(sorted.length / 2); return sorted.length % 2 ? sorted[m] : (sorted[m - 1] + sorted[m]) / 2; }
function computeScores(findings) {
    const impactWeights = { critical: 5, serious: 3, moderate: 2, minor: 1 };
    const counts = { critical: 0, serious: 0, moderate: 0, minor: 0, incomplete: 0, totalViolations: 0 };
    findings.forEach(f => {
    ...
    const buckets = { Perceivable: 0, Operable: 0, Understandable: 0, Robust: 0 };
    findings.forEach(f => {
        const r = f.rule_id;
        let key = 'Robust';
        if (/color|image|text-alt|contrast|audio|video|time|pause|blink/i.test(r))
            key = 'Perceivable';
        else if (/keyboard|focus|link|target|timing|gesture|pointer|trap|bypass/i.test(r))
            key = 'Operable';
        else if (/label|name|language|error|help|instructions|heading|structure/i.test(r))
            key = 'Understandable';
        else
            key = 'Robust';
    ...
    const toScore = (d) => Math.max(0, base - d);
    const scores = {
...
async function scanUrl(url, viewport, headers, auth) {
    const launchOpts = {
        headless: 'new',
    ...
    try {
        ...
        const findings = [];
        const extractFindings = (items, isViolation) => {
            for (const v of items) {
                const rule_id = v.id;
                const advice = (v.helpUrl || v.help || undefined);
                const sc = Array.isArray(v.tags) ? (v.tags.find((t) => /wcag(\d{1,2}([a-z]?))/.test(t)) || undefined) : undefined;
...
async function persistScan(scanId, orgId, siteId, urls, viewport, headers, auth, extraMeta = {}) {
    if (IN_MEMORY_MODE) {
        scansMem.push({
            id: scanId,
            org_id: orgId,
...
async function updateScan(scanId, data) {
    ...
    await pool.query('UPDATE scans SET status = COALESCE($2,status), partial = COALESCE($3,partial), counts = COALESCE($4,counts), scores = COALESCE($5,scores), sample_stats = COALESCE($6,sample_stats), meta = $7 WHERE id = $1', [
        ...
        data.status ?? null,
        data.partial ?? null,
        data.counts ? data.counts : null,
        data.scores ? data.scores : null,
        data.sample_stats ? data.sample_stats : null,
        metaMerged
    ]);
}
async function insertFindings(scanId, findings) {
    if (IN_MEMORY_MODE) {
    ...
    findings.forEach((f, i) => {
        ...
        values.push(scanId, f.url, f.rule_id, f.sc || null, f.impact || null, JSON.stringify(f.selectors || []), f.excerpt || null, f.contrast_ratio ?? null, f.advice || null, f.partial ?? false);
    });
...
async function getFindingsByScan(scanId) {
    if (IN_MEMORY_MODE) {
...
if (!IN_MEMORY_MODE) {
    const worker = new Worker('scans', async (job) => {
        ...
        const workers = Array.from({ length: Math.min(SCAN_PAGE_CONCURRENCY, queue.length) }, () => (async () => {
            for (;;) {
                ...
                try {
                    ...
                    if (!chromeVersion && browserVersion)
        ...
        const stats = { min: n ? sorted[0] : 0, median: n ? percentile50(sorted) : 0, mean: n ? Number((sorted.reduce((s, v) => s + v, 0) / n).toFixed(2)) : 0, max: n ? sorted[n - 1] : 0, n };
...
// Exports helpers
async function ensureArtifactsDir(scanId) {
...
function hmacToken(scanId, filename, exp) {
    return crypto.createHmac('sha256', SIGNING_SECRET).update(`${scanId}|${filename}|${exp}`).digest('hex');
}
function makeSignedUrl(baseUrl, scanId, filename, ttlSec = 3600) {
    const now = Math.floor(Date.now() / 1000);
...
async function loadWcagMapping() {
    try {
...
async function saveArtifact(scanId, type, filename, buffer) {
...
async function generateJsonArtifact(scan, findings) {
    const payload = {
        meta: {
            orgId: scan.org_id,
            siteId: scan.site_id,
...
async function generateCsvArtifact(scan, findings) {
...
async function generateDocxArtifact(scan, findings) {
    // Génère un DOCX minimal (OOXML) via JSZip, sans dépendances externes.
    const escapeXml = (s) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const scores = scan.scores || {};
    const stats = scan.sample_stats;
    // Versions — privilégier celles persistées dans le scan, avec repli sur l'env
    const axeVersion = (scan.meta?.axe_version ?? AXE_VERSION ?? 'unknown');
    const chromeRaw = scan.meta?.chrome_version ?? undefined;
    const chromeVersion = chromeRaw ? (chromeRaw.match(/\d+(?:\.\d+)*/)?.[0] ?? chromeRaw) : 'unknown';
    const pagesCount = Array.isArray(scan.urls) ? scan.urls.length : (Array.isArray(scan.meta?.urls) ? scan.meta.urls.length : (typeof scan.meta?.urls_count === 'number' ? scan.meta.urls_count : undefined));
    const lines = [
...
async function getPreviousMajorSet(siteId, excludeScanId) {
    ...
    for (const row of prev.rows)
        set.add(`${row.url}|${row.rule_id}`);
    return set;
}
async function countNewMajors(scan) {
...
async function getPreviousCompletedScan(siteId, excludeScanId) {
    const r = await pool.query(`SELECT id, scores FROM scans WHERE site_id = $1 AND status = 'completed' AND id <> $2 ORDER BY created_at DESC LIMIT 1`, [siteId, excludeScanId]);
    return r.rows.length ? r.rows[0] : null;
}
function computeScoreDropPct(prevOverall, curOverall) {
    if (prevOverall == null || curOverall == null)
        return 0;
    if (prevOverall <= 0)
        return 0;
...
async function countNewSCViolations(scan, scList) {
    const prevSet = await getPreviousMajorSet(scan.site_id, scan.id);
    const cur = await pool.query(`SELECT url, rule_id, sc FROM findings WHERE scan_id = $1 AND impact IN ('critical','serious') AND sc = ANY($2)`, [scan.id, scList]);
    let c = 0;
    for (const row of cur.rows) {
        const key = `${row.url}|${row.rule_id}`;
        if (!prevSet.has(key))
...
async function postToSlack(webhookUrl, title, overall, newMajors, artifactsText, triggers = [], dropPct) {
    ...
    if (triggers.length)
        parts.push(`Déclencheurs: ${triggers.join(', ')}`);
    if (dropPct != null && dropPct > 0)
        parts.push(`Baisse de score: -${dropPct}%`);
...
// Worker EXPORTS
if (!IN_MEMORY_MODE) {
    const exportsWorker = new Worker('exports', async (job) => {
        ...
        const triggers = [];
        if (newMajors > 0)
            triggers.push(`majors.new:${newMajors}`);
        if (dropPct >= ALERT_SCORE_DROP_PCT)
            triggers.push(`score.drop:${dropPct}%`);
        const sc143 = await countNewSCViolations(scan, ['1.4.3']);
        if (sc143 > 0)
            triggers.push(`wcag.1.4.3.new:${sc143}`);
        const sc1411 = await countNewSCViolations(scan, ['1.4.11']);
        if (sc1411 > 0)
            triggers.push(`wcag.1.4.11.new:${sc1411}`);
...
if (AUTH_ENABLED) {
    ...
    app.get('/auth/callback/:provider', async (req, res) => {
        try {
            ...
            if (!cookieState)
                return res.status(400).json({ error: 'state_cookie_missing' });
            const parts = cookieState.split('.');
            if (parts.length < 3)
                return res.status(400).json({ error: 'state_invalid' });
            const combo = `${parts[0]}.${parts[1]}`;
            const sig = parts.slice(2).join('.');
            if (signHmac(combo) !== sig)
                return res.status(400).json({ error: 'state_tampered' });
            if (parts[0] !== state)
...
async function isOrgSubscriptionActive(orgId) {
    ...
    if (status !== 'active')
        return false;
    if (current_period_end && new Date(current_period_end).getTime() < Date.now())
        return false;
    return true;
}
// Billing routes
app.post('/billing/checkout', async (req, res) => {
    try {
        if (FREE_MODE)
            return res.status(403).json({ error: 'free_mode' });
        if (!stripe)
            return res.status(503).json({ error: 'stripe_not_configured' });
        const { orgId: bodyOrgId, priceId, qty = 1 } = req.body || {};
        const orgId = bodyOrgId || req.headers['x-org-id'];
        if (!orgId)
...
app.post('/billing/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    try {
        if (FREE_MODE)
            return res.status(503).json({ error: 'free_mode' });
...
app.post('/test/seed', async (req, res) => {
    ...
    try {
        const orgId = req.body?.orgId || 'test-org';
        const siteId = req.body?.siteId || 'test-site';
        const viewport = req.body?.viewport || { w: 1366, h: 768 };
        const urls = Array.isArray(req.body?.urls) && req.body.urls.length > 0 ? req.body.urls : ['https://example.com'];
        const scanIds = [];
        const makeCounts = (critical, serious, moderate, minor, incomplete) => ({
            critical, serious, moderate, minor, incomplete,
            totalViolations: critical + serious + moderate + minor,
        });
        const makeScores = (overall) => ({
            Perceivable: overall,
            Operable: overall,
...
app.post('/scans', async (req, res) => {
    ...
    const perPageMs = 30000;
    const jobTimeout = Math.max(60000, perPageMs * body.urls.length + 15000);
    const jobsOptions = { attempts: 2, backoff: { type: 'exponential', delay: 1000 }, removeOnComplete: true, removeOnFail: false };
...
app.get('/scans/:id/artifacts', async (req, res) => {
    ...
    try {
        ...
        res.json({ items: r.rows.map((x) => ({ name: x.filename, type: x.type, hash: x.sha256, createdAt: x.created_at })) });
    }
    catch (e) {
...
app.post('/scans/:id/evidence', async (req, res) => {
    ...
    try {
        ...
        const baseUrl = process.env.PUBLIC_BASE_URL || '';
        if (!baseUrl)
            return res.json({ filename: meta.filename });
...
app.post('/schedules', async (req, res) => {
    try {
        ...
        if (type !== 'every' && type !== 'cron')
            return res.status(400).json({ error: 'invalid_type' });
        if (type === 'every' && (!every_ms || Number(every_ms) <= 0))
            return res.status(400).json({ error: 'invalid_every_ms' });
        if (type === 'cron' && !cron)
            return res.status(400).json({ error: 'invalid_cron' });
        // Access control: ensure requester is member of the org when auth is enabled
        if (AUTH_ENABLED) {
            const ok = await requireOrgMemberFromReq(req, res, orgId);
            if (!ok)
        ...
        const id = uuidv4();
        const r = await pool.query(`INSERT INTO schedules (id, org_id, site_id, type, every_ms, cron, timezone, active, run_limit)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id, org_id, site_id, type, every_ms, cron, timezone, active, run_limit AS limit, created_at`, [id, orgId, siteId, type, every_ms ?? null, cron ?? null, timezone ?? null, active === true, limit ?? null]);
        const row = r.rows[0];
        await upsertSchedulerForRow(row);
...
app.patch('/schedules/:id', async (req, res) => {
    try {
        ...
        if (next.type !== 'every' && next.type !== 'cron')
            return res.status(400).json({ error: 'invalid_type' });
        if (next.type === 'every' && (!next.every_ms || Number(next.every_ms) <= 0))
            return res.status(400).json({ error: 'invalid_every_ms' });
        if (next.type === 'cron' && !next.cron)
            return res.status(400).json({ error: 'invalid_cron' });
        ...
        if (!canProceed && next.active === true)
            return res.status(402).json({ error: 'subscription_inactive' });
        const r = await pool.query(`UPDATE schedules SET type=$2, every_ms=$3, cron=$4, timezone=$5, active=$6, run_limit=$7 WHERE id=$1 RETURNING id, org_id, site_id, type, every_ms, cron, timezone, active, run_limit AS limit, created_at`, [id, next.type, next.every_ms ?? null, next.cron ?? null, next.timezone ?? null, next.active === true, next.limit ?? null]);
        const row = r.rows[0];
        if (row.active)
            await upsertSchedulerForRow(row);
        else
...
async function purgeExpiredArtifacts() {
    ...
    try {
        ...
        const scanDirs = await readdir(ARTIFACTS_DIR, { withFileTypes: true }).catch(() => []);
