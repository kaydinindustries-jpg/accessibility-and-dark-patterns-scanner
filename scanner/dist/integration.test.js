...
async function assertDocxContainsWatermark(buf, needle) {
...
async function main() {
    ...
    const bill = await getJSON(`${BASE_URL}/billing/status?orgId=${encodeURIComponent(orgId)}`);
    if (!bill || bill.free !== true)
        throw new Error('expected_free_mode_true');
    if (bill.subscription !== null)
        throw new Error('expected_subscription_null_in_free_mode');
    console.log('[test] checking billing checkout is disabled in FREE_MODE');
    {
        ...
        if (r.status !== 403)
            throw new Error(`expected_checkout_403_in_free_mode, got ${r.status}`);
        const txt = await r.text();
        if (!txt.includes('free_mode'))
            throw new Error('expected_free_mode_marker_in_checkout_response');
    }
    const body = {
        orgId,
        siteId: 'test-site',
    ...
    const seedRes = await postJSON(`${BASE_URL}/test/seed`, body);
    if (!seedRes.ok || !seedRes.scanIds || seedRes.scanIds.length !== 3)
        throw new Error('seed_failed');
    console.log('[test] verifying list scans majors and timeline');
    const list = await getJSON(`${BASE_URL}/sites/${encodeURIComponent(seedRes.siteId)}/scans?limit=10&orgId=${encodeURIComponent(orgId)}`);
    if (!list.items || list.items.length < 3)
        throw new Error('list_scans_insufficient');
    const majorsSum = list.items.reduce((acc, it) => acc + (it.majors || 0), 0);
    if (majorsSum < 1)
        throw new Error('majors_expected_at_least_1');
    console.log('[test] verifying diff for last scan');
    const lastScanId = list.items[0].id;
    const diff = await getJSON(`${BASE_URL}/scans/${encodeURIComponent(lastScanId)}/diff?orgId=${encodeURIComponent(orgId)}`);
    if (!diff.summary)
        throw new Error('diff_summary_missing');
